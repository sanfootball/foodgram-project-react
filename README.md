...
Прошу прощения у Вас, этот файл, также как и data_in_tags.py я просто забыл удалить. Они нужны были мне только в начале, чтобы то, что я создал руками, данные в базе, подгрузить в общий файл ingredients.csv(и созданный мной tags.csv). Это еще когда я sqlite работал. А больше они нигде не используются. Файл, который нужен нам для импорта csv файла в базу другой, просто с похожим названием, он на месте, в папке managments. Прошу прощения, что сразу не удалил.
